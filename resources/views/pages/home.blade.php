    ...
    <div class="chat-list-container">
        @foreach ($conversations as $item)
            <a id="conversation-{{ $item->id }}"
                class="item-user text-decoration-none d-flex justify-content-between conversation-link"
                data-id="{{ $item->id }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.is_member) {
                                    ...
                                    console.log('Có tin nhắn mới từ cuộc trò chuyện ' + e
                                        .notification.id);
                                }
                            }
                        });
            ...
            function updateConversationList(notification) {
                ...
                const avatarUrl = notification.is_group == false ? (notification.friend.avatar ? notification.friend
                    .avatar : '/assets/images/avatar_default.jpg') : (notification.avatar ? notification
                    .avatar : '/assets/images/avatar_default_group.jpg');
                ...
                const time = notification.latestMessage ? notification.latestMessage.time_diff :
                    `Tạo ${notification.time_diff}`;
                ...
                fetch(`/conversation/${conversationId}/members?page=${page}&per_page=${perPage}`)
                    ...
                    .then(data => {
                        ...
                        if (page === 1) {
                            ...
                            ''; // Xóa danh sách thành viên hiện tại nếu là trang đầu tiên
                        }
                    ...
                    `;
                        ...
                        document.querySelectorAll('.remove-member-btn').forEach(button => {
                            button.addEventListener('click', function() {
                                ...
                                const confirmRemoveMemberModal = new bootstrap.Modal(document
                                    .getElementById('confirmRemoveMemberModal'));
                ...
                fetch(`/conversation/${conversationId}/remove-member`, {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            ...
                            1); // Tải lại danh sách thành viên từ trang đầu tiên sau khi xóa
                            const confirmRemoveMemberModal = bootstrap.Modal.getInstance(document
                                .getElementById('confirmRemoveMemberModal'));
